...
<head>
    ...
    <title>Bugify - Guest Playlists</title>
...
<body class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div class="mb-6 p-4 rounded bg-yellow-300 text-yellow-800">
            You are in guest mode. Your playlists are stored temporarily and will be lost unless you log in and save them.
        </div>
        <br>
        ...
        {{-- Aanmaakformulier --}}
        <form id="playlistForm" method="POST" action="{{ route('guest.playlists.store') }}" class="mb-12 bg-background-surface p-6 rounded-xl shadow-lg">
            @csrf
        ...
        <section>
            <h2 class="text-3xl font-bold mb-6 text-spotify-green">Existing Playlist</h2>

            @if(empty($playlist))
                <p class="text-gray-400">You have no playlist yet.</p>
            @else
                @php
                    $playlistMusic = $playlist['music'] ?? [];
                    $totalDuration = collect($playlistMusic)->sum('duration');
                ...
                <details class="bg-background-surface rounded-xl shadow p-4">
                    <summary class="cursor-pointer font-semibold text-xl truncate text-spotify-green select-none">
                        {{ $playlist['name'] }} ({{ count($playlistMusic) }} songs,
                    ...
                    <button
                        onclick='editPlaylist("{{ $playlist["id"] ?? 0 }}", @json($playlist["name"]), @json(collect($playlistMusic)->pluck("id")))'
                    ...
                    <form method="POST" action="{{ route('guest.playlists.destroy') }}" onsubmit="return confirm('Are you sure you want to delete this playlist?')" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
